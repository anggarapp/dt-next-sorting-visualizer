...
import { algorithmOptions, generateAnimationArray, sortingAlgorithmsData } from "@/lib/util";
...
export default function Home() {
  ...
  return <main className="absolute top-0 h-screen w-screen z-[-2] bg-[#C0C0C0] bg-[radial-gradient(#ffffff33_1px, #150229_1px)] bg-[size:40px_p0px]">
    <div className="flex h-full justify-center">
      <div id="content-container" className="flex max-w-[1020px] w-full flex-col lg:px-0 px-4">
        <div className="h-[66px] relative flex items-center justify-between w-full">
          <h1 className="text-2xl font-light hidden md:flex">Sorting Visualizer</h1>
          <div className="flex items-center justify-center gap-4">
            ...
            <button className="flex items-center justify-center" onClick={handlePlay}>
              {requiresReset ? (
                <RxReset className="text-gray-700 h-8 w-8" />
              ) : (
                <FaPlayCircle className="text-system-orange500 h-8 w-8" />
              )}
            </button>
          </div>
          <div className="hidden sm:flex absolute top-[120%] left-0 w-full">
            <div className="flex w-full text-gray-700 p-4 rounded border border-orange-600 bg-orange-300 bg-opacity-10 gap-6">
          ...
          <div className="absolute bottom-[32px] w-full mx-auto left-0 right-0 flex justify-center items-end">
            {
              arrayToSort.map((value, index) => (
                <div key={index} className="array-line relative w-1 mx-0.5 shadow-lg opacity-70 rounded-lg default-line-color" style={{ height: `${value}px` }}>
                </div>
              ))
            }
          </div>
